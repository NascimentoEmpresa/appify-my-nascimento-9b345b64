...
export function useCapaUpdate(empresaId: string) {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      // (posicao não é sobrescrita em "Perdemos": perder não implica 2º lugar)
      if (changes.status && changes.status !== current.status && current.grade_id) {
        ...
        if (gradeChanges) {
          ...
          gradeHistorico.push({ ts: now, usuario, campo: "Fase", de: (gradeAtual as any)?.fase ?? "—", para: "Finalizada" });
