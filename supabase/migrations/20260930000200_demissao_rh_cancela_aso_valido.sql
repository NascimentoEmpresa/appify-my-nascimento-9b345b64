-- =========================================================================
-- Demissão: o RH também cancela com a solicitação em "ASO válido"
--
-- PEDIDO (21/09/2026, Pablo) — complemento da mig 198. "ASO válido" é o
-- verde da fila: o SST concluiu sem exame demissional porque o ASO do
-- colaborador ainda vale (< 90 dias). A reconsideração pode chegar depois
-- disso também, e o RH ficava sem o botão.
--
-- O QUE MUDA em demissao_cancelar
--   • RH cancela também em 'ASO válido'.
--   • NÃO avisa o SST nesse caso: não há agendamento a desmarcar (a
--     notificação da 198 continua só pros três status com exame em jogo).
--     O fio da conversa registra que foi cancelada depois do ASO válido.
--   • O solicitante continua sem cancelar ali (manda falar com o RH).
--
-- Idempotente. Aplicar no banco do app. ROLLBACK: reaplicar a 198.
-- =========================================================================
...
BEGIN
  ...
  v_aso_valido := d.status = 'ASO válido';
  ...
  IF v_sou_rh AND (d.status = 'Pendente RH' OR v_no_sst OR v_aso_valido) THEN
    v_quem := 'pelo RH';
  ELSIF v_sou_solic THEN
    IF v_aso_valido THEN
      RAISE EXCEPTION 'O SST já concluiu esta solicitação (ASO válido). Para desfazer, fale com o RH.';
    END IF;
  ...
  VALUES ('demissao', p_id::text, coalesce(v_nome, v_email), v_email,
          ...
          || CASE WHEN v_no_sst THEN ' com a solicitação no SST (' || d.status || ') — SST avisado para cancelar o agendamento do ASO.'
                  WHEN v_aso_valido THEN ' com a solicitação já concluída pelo SST (ASO válido — não havia exame a desmarcar).'
                  ELSE '.' END
...
-- ROLLBACK
-- =========================================================================
-- Reaplicar demissao_cancelar da 20260930000198.
-- NOTIFY pgrst, 'reload schema';
