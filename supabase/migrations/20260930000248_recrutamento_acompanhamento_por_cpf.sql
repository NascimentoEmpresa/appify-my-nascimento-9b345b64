-- =========================================================================
-- ACOMPANHAMENTO (Recrutamento) — versão por CPF, que SUBSTITUI a da 245
--
-- A 20260930000245 entrou na main com a chave por EMPREGADOS.ID e sem os
-- candidatos que ainda não estão na Senior. No mesmo dia (25/09/2026) o
-- pedido mudou — "deve aparecer todos colaboradores admitidos nos últimos
-- 90 dias (EMPREGADOS) e também os concluídos do Recrutamento; se não vier
-- da EMPREGADOS: Ainda não admitido no sistema Senior" — e a chave passou a
-- ser o CPF. Migration mergeada não se edita (R4): a versão nova vem aqui.
--
-- No banco do app a 245 antiga NUNCA foi aplicada (caiu por bloqueio do
-- backup); esta foi aplicada no lugar dela. Onde a 245 antiga tenha rodado,
-- as funções com a assinatura antiga (bigint) são removidas abaixo — as
-- tabelas antigas, se existirem com empregado_id como chave, precisam ser
-- recriadas à mão (estavam vazias; nenhum ambiente conhecido tem isso).
-- =========================================================================
