-- =========================================================================
-- CORREÇÃO da 20260930000238_contrato_quadro_postos (25/09/2026)
--
-- A 238 não aplica: o bloco que procura a UNIQUE antiga compara
-- array_agg(att.attname) — que é name[] — com ARRAY['contrato_id', ...]
-- (text[]), e o Postgres recusa ("operator does not exist: name[] = text[]").
-- Como a transação inteira volta, NADA da 238 ficou no banco.
--
-- A 238 já está na main e migration mergeada não se edita (R4). Esta é a
-- 238 inteira, com o único ajuste: attname::text no array_agg. Aplicar ESTA
-- no lugar da 238. Idempotente.
-- =========================================================================
