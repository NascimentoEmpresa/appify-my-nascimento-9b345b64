-- =========================================================================
-- RH › Colaboradores: filtros multi-valor (empresa, contrato, situação) e
-- filtro de cargo
--
-- Pedido do Pablo em 15/09/2026: "selecionar mais de uma empresa, contrato
-- ou situação; menu suspenso para filtrar cargo, também podendo selecionar
-- mais de um".
--
-- As duas RPCs (rh_colaboradores_dashboard / rh_colaboradores_lista)
-- continuam recebendo TEXTO em _empresa/_contrato/_situacao — só que agora
-- o texto pode trazer vários valores separados por U+001F (unit separator,
-- um caractere que não aparece em nome de empresa, contrato, cargo ou
-- situação). '' segue sendo "todos". Assim a tela antiga (um valor só)
-- continua funcionando sem mudar nada.
--
-- Entra o parâmetro _cargo (mesma regra) e 'cargos' em opcoes, pro menu
-- suspenso da tela. Na lista, "saída" (quem saiu no mês) vale quando
-- QUALQUER situação escolhida é de desligamento.
--
-- Assinatura mudou (parâmetro novo): as versões antigas são derrubadas
-- antes, senão o PostgREST vê duas funções com o mesmo nome e recusa a
-- chamada por ambiguidade.
--
-- Corpo = o que estava no banco em 15/09/2026 (20260930000011) + os
-- trechos acima. Idempotente. Aplicar no banco do app.
-- =========================================================================

DROP FUNCTION IF EXISTS public.rh_colaboradores_dashboard(integer, integer, text, text, text, text);
DROP FUNCTION IF EXISTS public.rh_colaboradores_lista(integer, integer, text, text, text, text, integer, integer);
...
CREATE OR REPLACE FUNCTION public.rh_colaboradores_lista(_ano integer, _mes integer, _empresa text DEFAULT ''::text, _contrato text DEFAULT ''::text, _situacao text DEFAULT ''::text, _busca text DEFAULT ''::text, _offset integer DEFAULT 0, _limite integer DEFAULT 50, _cargo text DEFAULT ''::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
...
REVOKE ALL ON FUNCTION public.rh_colaboradores_dashboard(integer, integer, text, text, text, text, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.rh_colaboradores_dashboard(integer, integer, text, text, text, text, text) TO authenticated;
REVOKE ALL ON FUNCTION public.rh_colaboradores_lista(integer, integer, text, text, text, text, integer, integer, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.rh_colaboradores_lista(integer, integer, text, text, text, text, integer, integer, text) TO authenticated;

NOTIFY pgrst, 'reload schema';

-- Conferência: duas empresas de uma vez.
-- SELECT (public.rh_colaboradores_dashboard(2026, 9, E'HAGG\x1FNASCIMENTO'))->'total';

-- =========================================================================
-- ROLLBACK
--   DROP as duas funções com a assinatura nova e reaplicar o bloco das RPCs
--   da 20260930000011_colaboradores_ver_salario.sql.
-- =========================================================================
