-- =====================================================================
-- SIS-2026-0385 — aprovação global sem esbarrar no UNIQUE (empresa_id, cnpj_cpf)
--
-- A 20260930000112 separou a busca por escopo (global só casa com global,
-- local só casa com local), mas public.fornecedor continua com
-- UNIQUE (empresa_id, cnpj_cpf) desde a criação da tabela — o fornecedor
-- global também ocupa a vaga (empresa_id, cnpj) da empresa de origem. Dois
-- casos quebravam a aprovação com "duplicate key":
--
--   1) "Todas as empresas" com o CNPJ já cadastrado como LOCAL na empresa de
--      origem (empresa ativa): não achava global, tentava INSERT ali.
--      Agora esse cadastro local é PROMOVIDO a global e atualizado — mesmo
--      padrão do FornecedorDialog, onde tornar global é ligar is_global no
--      próprio registro, sem criar outro.
--   2) Empresa específica com o CNPJ já GLOBAL com essa mesma empresa_id:
--      não achava local, tentava INSERT. Agora recusa com mensagem clara
--      (aprovar como "Todas as empresas" atualiza o global) — rebaixar o
--      global a local continua proibido, como na 0112.
--
-- Mesma assinatura da 0112 (CREATE OR REPLACE basta). A tela espelha a regra
-- em decidirDestino (src/hooks/useFornecedorCadastro.ts).
--
-- ROLLBACK:
--   Reaplicar a versão de public.sup_forn_aprovar da migration
--   20260930000112_fornecedor_aprovacao_todas_empresas.sql.
-- =====================================================================
...
DECLARE
  ...
  v_global_na_empresa boolean := false;
BEGIN
  ...
  IF v_is_global THEN
    -- 1º o global que já existe; 2º o local da empresa de origem, que é
    -- promovido (o UNIQUE não deixa criar outro registro nessa empresa).
    ...
    IF v_id IS NULL THEN
      SELECT f.id INTO v_id
        FROM public.fornecedor f
       WHERE NOT f.is_global
         AND f.empresa_id = p_empresa_id
         AND regexp_replace(coalesce(f.cnpj_cpf, ''), '\D', '', 'g')
           = regexp_replace(coalesce(v_p.cnpj_cpf, ''), '\D', '', 'g')
       ORDER BY f.created_at
       LIMIT 1;
      v_promover := v_id IS NOT NULL;
    END IF;
  ELSE
    SELECT f.id, f.is_global INTO v_id, v_global_na_empresa
      FROM public.fornecedor f
     WHERE f.empresa_id = p_empresa_id
       AND regexp_replace(coalesce(f.cnpj_cpf, ''), '\D', '', 'g')
         = regexp_replace(coalesce(v_p.cnpj_cpf, ''), '\D', '', 'g')
     ORDER BY f.is_global, f.created_at
     LIMIT 1;

    IF coalesce(v_global_na_empresa, false) THEN
      RAISE EXCEPTION 'Este CNPJ já está cadastrado para todas as empresas. Aprove como "Todas as empresas" para atualizar o cadastro global.';
    END IF;
  END IF;
  ...
  ELSE
    UPDATE public.fornecedor f SET
      is_global          = f.is_global OR v_promover,
